@php
     ...
     * ✅ GROUP BY TEST TYPE
     * We try:
     * 1) testType relation name
     * 2) fallback to snapshot field if you ever stored one
     * 3) fallback text
     */
    $byType = $order->items->groupBy(function ($it) {
        return $it->testType?->name
            ?? ($it->test_type_name_snapshot ?? null)
            ?? 'Other Type';
    });
@endphp
...
<head>
    ...
    font-size: 10px;
    color: #374151;
    margin-bottom: 6px; /* space before line */
}
    ...
    margin-bottom: 6px; /* space before doctors */
}

        .footer-doctors{
    font-size: 9.4px; /* small so it fits */
    color: #111827;
}
...
.footer-wrap{
    ...
    bottom: 18px;
        ...
        .proc { color:#9ca3af; font-style:italic; }
    </style>
</head>

<body>

@foreach($byType as $typeName => $typeItems)
@php
    // Group THIS type's items by category
...
<div class="page">
    <div class="content">
        ...
        {{-- ✅ Test Type heading (each page) --}}
        <div class="type-title">{{ $typeName }}</div>
        <div class="type-sub">This page contains results for this Test Type only.</div>

        {{-- Tests for this type (grouped by category) --}}
            ...
            <table class="report">
                ...
                @foreach($sortedItems as $it)
                    @php
                        ...
                        // parent hint for SUB items (optional)
                        $parentName = null;
