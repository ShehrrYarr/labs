    ...
    <div class="content-wrapper">
        ...
        <div class="wrap">
            <div class="card">
                <div class="header">
                    <div>
                        <h2 style="margin:0;color:#0f172a;">Edit Sub Test</h2>
                    ...
                    <div style="display:flex;gap:10px;flex-wrap:wrap;">
                        ...
                        <form method="POST" action="{{ route('lab-tests.sub-tests.destroy', [$labTest, $subTest]) }}"
                              onsubmit="return confirm('Delete this sub test?')">
                            @csrf
                            @method('DELETE')
                ...
                <form method="POST" action="{{ route('lab-tests.sub-tests.update', [$labTest, $subTest]) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid">
                        <div class="section">
                            ...
                            <div class="row2">
                                <div>
                                    ...
                                    <input type="text" name="test_name" value="{{ old('test_name', $subTest->test_name) }}" required>
                                </div>
                                <div>
                                    <label>Test Code (optional)</label>
                                    <input type="text" name="test_code" value="{{ old('test_code', $subTest->test_code) }}">
                                </div>
                            </div>
                            ...
                            <div class="row2" style="margin-top:12px;">
                                <div>
                                    ...
                                    <select name="test_type_id" required>
                                        ...
                                        @foreach($types as $t)
                                            <option value="{{ $t->id }}" {{ old('test_type_id', $subTest->test_type_id) == $t->id ? 'selected' : '' }}>
                                                {{ $t->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="test_category_id">
                                        ...
                                        @foreach($categories as $c)
                                            <option value="{{ $c->id }}" {{ old('test_category_id', $subTest->test_category_id) == $c->id ? 'selected' : '' }}>
                                                {{ $c->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row2" style="margin-top:12px;">
                                <div>
                                    ...
                                    <input type="text" name="unit" value="{{ old('unit', $subTest->unit) }}">
                                </div>
                                <div>
                                    <label>Reference Range (optional)</label>
                                    <input type="text" name="reference_range" value="{{ old('reference_range', $subTest->reference_range) }}">
                                </div>
                            </div>
                            ...
                            <div class="row2" style="margin-top:12px;">
                                <div>
                                    ...
                                    <input type="text" name="reporting_time" value="{{ old('reporting_time', $subTest->reporting_time) }}">
                                </div>
                                <div>
                                    <label>Sort Order</label>
                                    <input type="number" min="0" name="sort_order" value="{{ old('sort_order', $subTest->sort_order ?? 0) }}">
                                </div>
                            </div>

                            <div style="margin-top:12px;">
                                <label>Description (optional)</label>
                                <textarea name="description">{{ old('description', $subTest->description) }}</textarea>
                            </div>

                            <div class="row2" style="margin-top:12px;">
                                <div>
                                    <label>Test Instruction (optional)</label>
                                    <textarea name="test_instruction">{{ old('test_instruction', $subTest->test_instruction) }}</textarea>
                                </div>
                                <div>
                                    <label>Additional Notes (optional)</label>
                                    <textarea name="additional_notes">{{ old('additional_notes', $subTest->additional_notes) }}</textarea>
                                </div>
                            </div>
                        ...
                        <div class="section">
                            ...
                            <div style="margin-bottom:12px;">
                                ...
                                @php
                                    $current = $subTest->test_case_image
                                        ? asset('storage/' . $subTest->test_case_image)
                                ...
                                <div class="imgBox">
                                    <img id="preview" class="preview" src="{{ $current }}">
                                    <div style="flex:1;">
                            ...
                            <div>
                                ...
                                <select name="required_equipment_id">
                                    ...
                                    @foreach($equipment as $eq)
                                        <option value="{{ $eq->id }}" {{ old('required_equipment_id', $subTest->required_equipment_id) == $eq->id ? 'selected' : '' }}>
                                            {{ $eq->name }}
                                        </option>
                                    @endforeach
                                </select>
