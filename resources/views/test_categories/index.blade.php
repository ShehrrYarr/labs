...
<div class="card">
    <div class="top">
        <div>
            <h2 style="margin:0;">Test Categories</h2>
            <div style="color:#64748b;font-size:13px;margin-top:4px;">Manage dynamic categories used inside Tests.</div>
        </div>
        <a class="btn" href="{{ route('test-categories.create') }}">+ Add Category</a>
    </div>
        ...
        <tbody>
            @forelse($categories as $cat)
                <tr>
                    ...
                    <td class="actions">
                        <a href="{{ route('test-categories.edit', $cat) }}">Edit</a>
                        <form action="{{ route('test-categories.destroy', $cat) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
